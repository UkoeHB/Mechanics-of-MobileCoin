...
At this point we have covered the conceptual foundation of SCP balloting. In addition to the prior rules and heuristics, a litany of non-essential optimizations were defined in \cite{stellar-consensus-protocol} and \cite{scp-ietf-draft}.
...
\begin{enumerate}
    ...
    \item Since in general the network moves to new ballot counters when lower counters are possibly stuck or not working, it is useful to define a catch-up mechanism for nodes who are at ballot counters lower than the network.% More accurately, given the prior optimization/complication it is now useful to define a catch-up mechanism.
